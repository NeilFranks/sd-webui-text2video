...
def process_modelscope_sequence(args_dict):
    args, video_args = process_args(args_dict)
    ...
    print(f"Git commit: {get_t2v_version()}")
    init_timestring = time.strftime('%Y%m%d%H%M%S')
    outdir_current = os.path.join(get_outdir(video_args.subdir_name), f"{init_timestring}")
    ...
    print('Starting text2video sequence thing')
    print('Pipeline setup')
    ...
    """
    SEPARATE PROMPT BY NEWLINES
    """
    prompt_list = args.prompt.split('\n')
    
    # Start the batch count loop
    state.job_count = len(prompt_list)
    pbar = tqdm(range(state.job_count), leave=False)

    for job_index in range(state.job_count):
        """
        START: CHANGE SOMETHING PER job
        """
        # args.cfg_scale += 3
        # print(f"cfg scale is {args.cfg_scale}")

        """
        END: CHANGE SOMETHING PER job
        """

        state.job_no = job_index + 1
        if state.skipped:
            state.skipped = False
        ...
        shared.state.job = f"job {job_index + 1} out of {state.job_count}"
        # TODO: move to a separate function
        if args.inpainting_frames > 0 and hasattr(args.inpainting_image, "name"):
            keys = T2VAnimKeys(SimpleNamespace(**{'max_frames': args.frames, 'inpainting_weights': args.inpainting_weights}), args.seed, args.inpainting_frames)
            images = []
        ...
        samples, _ = pipe.infer(prompt_list[job_index], args.n_prompt, args.steps, args.frames, args.seed if args.seed != -1 else -1, args.cfg_scale,
                                args.width, args.height, args.eta, cpu_vae, device, latents, skip_steps=skip_steps, mask=mask)

        outdir_current = os.path.join(get_outdir(video_args.subdir_name), f"{init_timestring}_{job_index}")
        print(f'text2video finished, saving frames to {outdir_current}')
