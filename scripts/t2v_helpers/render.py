...
import traceback
from modelscope.process_modelscope import process_modelscope, process_modelscope_sequence
...
def run(*args):
    ...
    args_dict = {component_names[i]: args[i] for i in range(0, len(component_names))}
...
def run_make_sequence(*args):
    ...
    try:
        print('text2video — The model selected is: ', args_dict['model_type'])
        if model_type == 'ModelScope':
            vids_pack = process_modelscope_sequence(args_dict)
        else:
            raise NotImplementedError(f"Unknown model type: {model_type}")
    except Exception as e:
        traceback.print_exc()
